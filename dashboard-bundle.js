...
${result.path}`);
  ...
  async function runFullRewardTest() {
    ...
    const priceResults = await Promise.all(
      identified.map(async (item) => {
        ...
        try {
          ...
          const sellOrders = (data.data?.sell || []).filter((o) => o.visible).sort((a, b) => a.platinum - b.platinum);
    ...
    popup.style.cssText = "position:fixed; top:100px; left:100px; background:#222; border:2px solid #4dd9ec; border-radius:8px; padding:20px; z-index:999999; color:#eee; font-family:sans-serif;";
    popup.innerHTML = `<h2>Reward Prices (${elapsed}ms) \u2014 Run #${window._testResults.length}</h2>` + priceResults.map((r) => r ? `<p><strong>${r.name}</strong>: ${r.price} plat</p>` : `<p>Unidentified</p>`).join("") + `<button id="closeTestPopup" style="margin-top:10px;">Close</button>`;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    triggerBtn.textContent = "\u{1F4F8} Capture Reward";
    triggerBtn.style.cssText = "position:fixed; bottom:20px; right:20px; z-index:999998; padding:12px 20px; background:#4dd9ec; color:#111; font-weight:bold; border:none; border-radius:8px; cursor:pointer; font-size:14px;";
